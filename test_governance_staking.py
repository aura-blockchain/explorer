...
class TestStakingService:
    ...
    def test_get_staking_params(self, mock_get, staking):
        ...
        assert "max_validators" in result or ("params" in result and "max_validators" in result["params"])
        assert "bond_denom" in result or ("params" in result and "bond_denom" in result["params"])
    ...
    def test_governance_single_proposal_endpoint(self, client):
        ...
        with patch('requests.get') as mock_get:
            mock_get.side_effect = [
                ...
                _MockResponse({
                    "tally": {"yes": "1000", "no": "0", "abstain": "0", "no_with_veto": "0"}
                })
            ]
...
class TestGovernanceStakingIntegration:
        ...
        with patch('requests.get') as mock_get:
            ...
            mock_get.side_effect = [
                _MockResponse({
                    "proposal": {
                        "proposal_id": "10",
                        "content": {"title": "Upgrade v2", "description": "Full details"},
                        "status": "PROPOSAL_STATUS_VOTING_PERIOD",
                        "total_deposit": [{"denom": "uaura", "amount": "50000000"}]
                    }
                }),
                _MockResponse({
                    "tally": {"yes": "8000000", "no": "1000000", "abstain": "500000", "no_with_veto": "500000"}
                })
            ]
    ...
    def test_complete_staking_workflow(self, client):
        ...
        with patch('requests.get') as mock_get:
            ...
            mock_get.return_value = _MockResponse({
                "pool": {"bonded_tokens": "500000000000", "not_bonded_tokens": "50000000000"}
            })
            ...
            mock_get.return_value = _MockResponse({
                "rewards": [
                    {"validator_address": "auravaloper1val1", "reward": [{"denom": "uaura", "amount": "50000"}]}
                ],
